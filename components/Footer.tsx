...
const Footer = ({ currentLang }: FooterProps) => {
  ...
  const pageLinks = [
    { label: currentLang === 'uz' ? 'Agentlik haqida' : currentLang === 'ru' ? 'Об агентстве' : 'About Agency', href: '/about' },
  ...
  return (
    <footer id="contacts" className="bg-green-900 text-white w-full">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 lg:gap-12">
          {/* Contacts */}
          <div>
            <div className="accent-line mb-4" />
            <h3 className="font-display text-xl font-bold mb-5">{t.footer.contacts}</h3>
            <div className="space-y-3">
              <div className="flex items-start">
                <svg className="w-4 h-4 mt-0.5 mr-3 flex-shrink-0 text-accent-400" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
                  <path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <p className="text-white/60 text-sm">{contactInfo.address[currentLang]}</p>
              </div>
              <div className="flex items-center">
                <svg className="w-4 h-4 mr-3 flex-shrink-0 text-accent-400" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
                  <path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                <a href={`tel:${contactInfo.phone.replace(/\s/g, '')}`} className="text-white/60 hover:text-white transition-colors text-sm" tabIndex={0}>
                  {contactInfo.phone}
                </a>
              </div>
              <div className="flex items-center">
                ...
                <a href={`mailto:${contactInfo.email}`} className="text-white/60 hover:text-white transition-colors text-sm" tabIndex={0}>
                  {contactInfo.email}
                </a>
              </div>
              <div className="flex items-start">
                ...
                <p className="text-white/60 text-sm">{contactInfo.workingHours[currentLang]}</p>
              </div>

              <div className="mt-6 pt-5 border-t border-white/10">
                <h4 className="text-sm font-semibold mb-3 text-white/80">{t.footer.socialMedia}</h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              <h3 className="font-display text-sm font-bold mb-4 text-white/90 uppercase tracking-wide">
                {currentLang === 'uz' ? 'Agentlik haqida' : currentLang === 'ru' ? 'Об агентстве' : 'About Agency'}
              </h3>
              <ul className="space-y-2">
                {pageLinks.slice(0, 5).map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} className="text-white/50 hover:text-white transition-colors text-sm" tabIndex={0}>
                      {link.label}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              ...
              <ul className="space-y-2">
                {pageLinks.slice(5, 13).map((link) => (
                  <li key={link.href}>
                    <Link href={addLangToUrl(link.href, currentLang)} className="text-white/50 hover:text-white transition-colors text-sm" tabIndex={0}>
                      {link.label}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              <h3 className="font-display text-sm font-bold mb-4 text-white/90 uppercase tracking-wide">
                {currentLang === 'uz' ? 'Bog\'lanish' : currentLang === 'ru' ? 'Контакты' : 'Contact'}
              </h3>
              <ul className="space-y-2">
        ...
        {/* Form + Map */}
        <div className="mt-10 pt-8 border-t border-white/10 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <h3 className="font-display text-lg font-bold mb-4">{t.footer.feedbackTitle}</h3>
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <label htmlFor="name-footer" className="block text-xs font-medium mb-1 text-white/60">{t.footer.name}</label>
                <input
                  type="text"
                  id="name-footer"
                  name="name"
                  ...
                  required
                  className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-accent-400/50 focus:border-accent-400/50 text-sm transition-colors"
                  tabIndex={0}
                />
              </div>
              <div>
                <label htmlFor="email-footer" className="block text-xs font-medium mb-1 text-white/60">{t.footer.email}</label>
                <input
                  type="email"
                  id="email-footer"
                  name="email"
              ...
              <div>
                <label htmlFor="message-footer" className="block text-xs font-medium mb-1 text-white/60">{t.footer.message}</label>
                <textarea
                  id="message-footer"
                  name="message"
                  ...
                  required
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-accent-400/50 focus:border-accent-400/50 resize-none text-sm transition-colors"
                  tabIndex={0}
                />
              </div>
              <button
                ...
                className="w-full bg-green-800 hover:bg-green-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-green-600/50 text-sm"
                tabIndex={0}
              >
                {isSubmitting ? t.footer.sending : t.footer.send}
              </button>
              {submitStatus === 'success' && (
                <div className="p-3 bg-primary-500/20 border border-primary-500/30 rounded-lg text-sm text-primary-200">
                  {t.footer.success}
                </div>
              )}
              {submitStatus === 'error' && (
          ...
          <div ref={mapContainerRef}>
            ...
            <div className="w-full h-48 sm:h-56 md:h-64 rounded-lg overflow-hidden bg-white/5 border border-white/10">
              {shouldLoadMap ? (
                <iframe
                ...
                <div className="h-full flex items-center justify-center text-white/30 text-sm">
                  {currentLang === 'uz' && 'Xarita yuklanmoqda...'}
